...
import { DatabaseLabelItem } from '../model/labe-iItem.model';
import { InstanceVMProfile } from '../model/vm.model'

import 'rxjs/add/operator/toPromise';

@Injectable()
export class DatabaseServiceList {
    ...
    getLabels() : Promise<DatabaseLabelItem[]> { 
    ...
    useType = this.dict.get({    //数据库类型
        owner : "GLOBAL",
        field : "USE_TYPE"
    });
    serviceLevel = this.dict.get({    //数据库服务级别
        owner : "GLOBAL",
        field : "SERVICE_LEVEL"
    });
    queryField = this.dict.get({    //数据库检索的字段列表，取数据字典的code返回
        owner : "GLOBAL_QUERY",
        field : "COMPUTE_INSTANCE"
    });
    addonType = this.dict.get({    //数据库附加服务类型
