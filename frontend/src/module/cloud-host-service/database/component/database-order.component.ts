...
export class DatabaseComponentOrder implements OnInit {
	...
	vmBasePrice: number = 0; //数据库一次性费用
	vmTotalPrice: number = 0; //数据库费用
	...
	constructor(
		...
		private service: DatabaseServiceOrder
	) {
		this.v.result = {};
		this.dux.reset();
	};
	...
	private getSkuMap(code: "vm" | "disk"): SkuMap[] {   //获取根据参数获取的sku数组 因为数据库的sku不止一个
	...
	private setVmPrice(): void {   //设置主机的价格
		...
		console.log("匹配到的数据库：", this.vmProduct)
	...
	addCart() {   //加入购物车
		this.submitCheck().then(payLoadArr => {
			...
			return this.service.addCart(payLoadArr).then(res => {
				...
				// this.router.navigateByUrl("database-service/database-list");
			}).catch(res => {
				this.layoutService.hide();
	...
	private payLoadFormat(): PayLoad[] {
		...
		this.payLoadArr = [];
		this.payLoadArr.push(payLoad);   //加入数据库的订单
	...
	buyNow() {
		this.submitCheck().then(payLoadArr => {
			...
			this.service.saveOrder(payLoadArr).then(res => {
				this.layoutService.hide();
				this.router.navigate(['database-service/cart-order/', JSON.stringify(res)]);
			}).catch(error => {
				this.layoutService.hide();
